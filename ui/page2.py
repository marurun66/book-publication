...
def run_2():
    ...
    st.markdown(
        ...
        줄거리를 입력하여 책 제목을 찾기 위해서는 다른 부가적인 정보는 제외하고 **줄거리**만을 요약할 필요가 있었습니다.  
        이를 위해 뉴스 요약에 특화된 **BART 모델** 중 한국어 요약에 최적화된 **KoBART** 모델을 사용하였으며,  
    ...
    책 줄거리의 유사도를 계산하기 위해 **FAISS**를 활용한 문장 벡터화, **코사인 유사도**를 기준으로 유저가 입력한 줄거리와 유사한 책을 빠르게 찾아낼 수 있는 시스템을 구현했습니다.
